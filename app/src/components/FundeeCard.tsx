...
export function FundeeCard({ fundee, priority = false }: FundeeCardProps) {
	...
	const handleImageError = (
		...
		console.error(`Image failed to load: ${fundee.logo}`);
		console.log(`Fundee ID: ${fundee.id}, Name: ${fundee.company_name}`);
		setImageError(true);
					...
					<CardContent className="p-0">
						<div className="relative h-40 bg-secondary/30 overflow-hidden">
							...
							<div className="absolute top-3 right-3">
								<Badge
									variant="secondary"
									className="flex items-center gap-1 font-medium px-2.5 py-1"
								>
						...
						<div className="grid grid-cols-2 w-full gap-3 text-sm">
							...
							<div className="col-span-2 flex flex-col gap-2 mt-1">
								<div className="flex gap-2 w-full">
									...
									<Badge
										variant="outline"
										className="bg-primary/5 border-primary/20 text-primary text-xs flex-1 text-center"
									>
				...
				<div className="px-4 pb-4 space-y-6 text-sm">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
						<div className="flex flex-col gap-2 text-sm text-foreground">
							...
							<div className="flex justify-between">
								...
								<span>
									${fundee.current_funding.toLocaleString()}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground font-medium">
								...
								<span>
									${fundee.funding_requested.toLocaleString()}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground font-medium">
								...
								<span>
									{fundee.total_credits_issued.toLocaleString()}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground font-medium">
									Expected Credits (5 Years):
								</span>
								<span>
									{fundee.expected_credits.toLocaleString()}
								</span>
							</div>
							<div className="flex justify-between">
								<span className="text-muted-foreground font-medium">
						...
						<div
							className="flex justify-center items-center bg-white/80 rounded-md p-2 shadow-sm border border-border/50"
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start mt-2">
						...
						<div className="flex flex-col gap-2 text-sm text-foreground">
							...
							<div className="flex flex-col">
								...
								<span>
									Longitude: {fundee.longitude}, Latitude:{" "}
									{fundee.latitude}
								</span>
							</div>
						</div>
					...
					<div className="flex flex-col gap-1 text-xs mt-4">
						<div>
							...
							{fundee.longitude}, Latitude: {fundee.latitude}
						</div>
						<div>
							<strong>Contact:</strong> {fundee.contact}
						</div>
						<div>
